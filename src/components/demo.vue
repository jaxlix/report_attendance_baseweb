...
import gpsUtiles from '@/coordTransform';
...
export default {
    ...
    props: ["type", "styles", "data", "clickAdd"], // type = point：点   polygon:范围  line:路线
    components: {
        checkPoint
    },
    watch: {
        data(val) {
            if (this.type == "polygon") {
                if (val.type == "add") {
                    this.showPolygon(val.data);
                } else if (val.type == "upd") {
                    this.showPolygon(val.data);
                }
            } else if (this.type == "line") {
                this.addLine(this.data);
        ...
        addLine(data) {
            if (data.length > 0) {
                data.forEach(d => {
                    let list = JSON.parse(d.jurisdictionCoordinates);
                    let name = d.jurisdictionName;
                    this.$arcgis.setPolyline(list, this.xqfwLayer);
                    let x = 0;
                    let y = 0;
                    list.forEach(i => {
                        x += i[0];
            ...
            if (this.$arcgis.all) {
                ...
                if (this.type == "point") {
                    ...
                    this.$arcgis.drowPolygon(this.map, e => {
                        ...
                        //坐标转换
                        var gpsArr = [];
                        e.geometry.rings[0].forEach(point => {
                            gpsArr.push(gpsUtiles.mercator2gps(point[0], point[1]))
                        });
                    ...
                    // 添加路线
                    this.$arcgis.drowLine(this.map, e => {
                        this._toolbar = e.target;
                        this.$arcgis.drowLine(this.map, e.target, e);

                        //坐标转换
                        var gpsArr = [];
                        e.geometry.paths[0].forEach(point => {
                            gpsArr.push(gpsUtiles.mercator2gps(point[0], point[1]))
                        });
